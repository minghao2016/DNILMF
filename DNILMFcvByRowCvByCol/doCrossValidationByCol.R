doCrossValidationByCol <- function(inMat, kfold = 10, numSplit = 5) {
  # INPUT
  # inMat: input matrix
  ...
  for (i in 1:numSplit) {
    #######
    folds <- getCvIndex(totNum = numCol, nfold = kfold) 
    #######
    for (j in 1:kfold) {
      ...
      ########################
      isTestCol <- triplet[, colIndex %in% currIndex]
      currIndex <- which(isTestCol)
      ########################
